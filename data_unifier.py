...
def list_all_class_names(args):
    ...
    unified_ids = {
        "bag": 0,
        "other trash facemask clustered cigarette litter straw lid pop tab scrap": 1,
                                ...
                                for word in name.split(" "):
                                    ...
                                    if word:
                                        if word in unified_id:
                                            ...
                                            if unified_id in from_old_to_new_id_mapping[dataset_path]:
                                                from_old_to_new_id_mapping[dataset_path][
                                                    old_id] = from_old_to_new_id_mapping[dataset_path][unified_id]

                                            else:
                                                from_old_to_new_id_mapping[dataset_path][old_id] = unified_ids[unified_id]
                                            remapped_items += 1
                                            temp = 1
                                            break
    ...
    simple_names_lookup = {
        "bag": "bag",
        "other trash facemask clustered cigarette litter straw lid pop tab scrap": "litter",
    ...
    with open(os.path.join(target_dataset_dir, "data.yaml"), "w") as file:
        yaml.dump({"names": list(simple_names.keys()), "nc": len(unified_ids), "train": "../train/images", "test": "../test/images", "val": "../valid/images"},
                  file)

    # dump the data.yaml file in the root folder
    with open("labels.yaml", "w") as file:
        yaml.dump({"names": list(simple_names.keys()), "nc": len(unified_ids), "train": f"{target_dataset_dir.replace("datasets", "..")}/train/images", "test": f"{target_dataset_dir.replace("datasets", "..")}/test/images", "val": f"{target_dataset_dir.replace("datasets", "..")}/valid/images"},
                  file)
                                ...
                                if class_id in from_old_to_new_id_mapping[dataset_path]:
                                    ...
                                    if not os.path.exists(target_image_path):
                                        ...
                                        try:
                                            ...
                                            shutil.copy(
                                                image_path_2, target_image_path)
    ...
    print(
        f"Discarded {discarded_items} / {remapped_items + discarded_items} classes")
